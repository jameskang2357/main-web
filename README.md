...
If you have AWS CLI configured with your credentials, you can use the automated setup script:
...
- Display all the values you need for GitHub Secrets

**Prerequisites for automated setup:**
- AWS CLI installed (`aws --version` to check)
- AWS credentials configured (`aws configure` to set up)

### Option A2: CloudFormation Template

Alternatively, you can use the provided CloudFormation template:

```bash
aws cloudformation create-stack \
  --stack-name github-s3-sync-setup \
  --template-body file://cloudformation-setup.yaml \
  --parameters ParameterKey=BucketName,ParameterValue=your-bucket-name \
               ParameterKey=IAMUserName,ParameterValue=github-s3-sync-user \
  --capabilities CAPABILITY_NAMED_IAM
```

After the stack is created, get the outputs (including access keys):
...
**Important:** The Secret Access Key is only available in the CloudFormation outputs immediately after stack creation. Save it securely!
